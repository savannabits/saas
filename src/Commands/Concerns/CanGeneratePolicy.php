...
namespace Savannabits\Saas\Commands\Concerns;
...
trait CanGeneratePolicy
{
    ...
    protected function generatePolicyPath(array $entity): string
    {
        ...
        $basePath = Str::of($path)
            ->replace(['Models', 'Entities'], ['Policies', 'Policies'])
    ...
    protected function generatePolicyStubVariables(array $entity): array
    {
        ...
        $stubVariables['resource_fqcn'] = $entity['fqcn'];
        $stubVariables['resource'] = Str::of($entity['fqcn'])->afterLast('\\');
        ...
        $stubVariables['namespace'] = Str::of($path)->contains(['vendor', 'src'])
            ? 'App\Policies'
            : Str::of($namespace)->replace(['Models', 'Entities'], ['Policies', 'Policies']); /** @phpstan-ignore-line */
