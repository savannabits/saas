...
use Savannabits\Saas\Contracts\HasPermissions;
use Savannabits\Saas\Facades\Armor;
use Savannabits\Saas\Filament\Access\Resources\RoleResource\Pages;
...
class RoleResource extends Resource implements HasPermissions
{
    ...
    {
        return [
            'index' => \Savannabits\Saas\Filament\Resources\RoleResource\Pages\ListRoles::route('/'),
            'create' => \Savannabits\Saas\Filament\Resources\RoleResource\Pages\CreateRole::route('/create'),
            'view' => \Savannabits\Saas\Filament\Resources\RoleResource\Pages\ViewRole::route('/{record}'),
            'edit' => \Savannabits\Saas\Filament\Resources\RoleResource\Pages\EditRole::route('/{record}/edit'),
        ];
    }
    ...
    *----------------------------------*/
    ...
    public static function getResourceEntitiesSchema(): ?array
    {
        ...
        return collect(Armor::getResources())->sortKeys()->reduce(function ($entities, $entity) {
            $entities[] = Forms\Components\Section::make()
                ...
                ->schema([
                    Forms\Components\Toggle::make($entity['resource'])
                        ->label(Armor::getLocalizedResourceLabel($entity['fqcn']))
    ...
    public static function getResourceEntityPermissionsSchema($entity): ?array
    {
        return collect(Utils::getResourcePermissionPrefixes($entity['fqcn']))->reduce(function ($permissions /** @phpstan ignore-line */, $permission) use ($entity) {
            $permissions[] = Forms\Components\Checkbox::make($permission . '_' . $entity['resource'])
                ->label(Armor::getLocalizedResourcePermissionLabel($permission))
    ...
    {
        $entitiesStates = collect(Armor::getResources())
            ->when(Utils::isPageEntityEnabled(), fn ($entities) => $entities->merge(Armor::getPages()))
            ->when(Utils::isWidgetEntityEnabled(), fn ($entities) => $entities->merge(Armor::getWidgets()))
    ...
    {
        collect(Armor::getResources())->each(function ($entity) use ($set, $state) {
        ...
        collect(Armor::getPages())->each(function ($page) use ($set, $state) {
            if (Utils::isPageEntityEnabled()) {
                $set($page, $state);
            }
        });

        collect(Armor::getWidgets())->each(function ($widget) use ($set, $state) {
    ...
    *----------------------------------*/
    ...
    {
        return collect(Armor::getPages())->sortKeys()->reduce(function ($pages, $page) {
            $pages[] = Forms\Components\Grid::make()
                ->schema([
                    Forms\Components\Checkbox::make($page)
                        ->label(Armor::getLocalizedPageLabel($page))
                        ->inline()
    ...
    *----------------------------------*/
    ...
    {
        return collect(Armor::getWidgets())->reduce(function ($widgets, $widget) {
            $widgets[] = Forms\Components\Grid::make()
                ->schema([
                    Forms\Components\Checkbox::make($widget)
                        ->label(Armor::getLocalizedWidgetLabel($widget))
                        ->inline()
    ...
    *----------------------------------*/
    ...
    protected static function getCustomEntities(): ?Collection
    {
        ...
        collect(Armor::getResources())->each(function ($entity) use ($resourcePermissions) {
        ...
        $entitiesPermissions = $resourcePermissions
            ->merge(Armor::getPages())
            ->merge(Armor::getWidgets())
            ->values();
